...
// -- Internal Function for DBS Halftoning --
// Update the low-pass Error Image
cv::Mat1f _altLpErr_(const cv::Mat1f lpErrImg, std::vector<cv::Vec3i> pos, int kernelSize, const cv::Mat1f gskMat2D) {
...
cv::Vec2i _minErrDec_(const cv::Mat1f resImg, const cv::Mat1f lpErrImg, cv::Vec2i centPos, int kernelSize, const cv::Mat1f gskMat2D) {
    ...
    for (int rdx = -movHalfK; rdx <= movHalfK; rdx++)
        for (int cdx = -movHalfK; cdx <= movHalfK; cdx++) {
            ...
            std::vector<cv::Vec6i> posVecs;  // {Row, Col, CentDistRow, CentDistCol, NbrDistRow, NbrDistCol}
...
cv::Mat3f _visErr_(cv::Mat1f errImg) {
...
cv::Mat1f DBS(const cv::Mat1f grayImg, int kernelSize, float sigma, int iters, bool verbose) {
    ...
    if (verbose) saveData::initVar(workFolder + "/Err"), saveData::imgMat(_visErr_(errImg), "errImg_0");
    ...
    for (int iter = 0; iter < iters; iter++) {
        for (cv::Vec2i workPix : randList) {
            ...
            cv::Vec2i posME = _minErrDec_(resImg, lpErrImg, {row, col}, kernelSize, gskMat2D);
            ...
            lpErrImg = _altLpErr_(lpErrImg, posAlt, kernelSize, gskMat2D);
        }
        ...
        if (verbose) saveData::initVar(workFolder + "/Err"), saveData::imgMat(_visErr_(errImg), "errImg_" + std::to_string(iter + 1));
...
cv::Mat1f RTBDBS(const cv::Mat1f grayImg, int kernelSize, float sigma, int iters, bool verbose) {
    ...
    for (int iter = 0; iter < iters; iter++) {
        ...
        if (verbose) saveData::initVar(workFolder + "/Iter"), saveData::imgMat(resImg, "resImg_" + std::to_string(iter + 1)), swapNum = 0;
        if (verbose) saveData::initVar(workFolder + "/Err"), saveData::imgMat(_visErr_(errImg), "errImg_" + std::to_string(iter + 1));
        if (verbose) saveData::initVar(workFolder + "/LpErr"), saveData::imgMat(_visErr_(lpErrImg), "lpErrImg_" + std::to_string(iter + 1));
        if (verbose) saveData::initVar(workFolder);
    }
    return resImg;
}
